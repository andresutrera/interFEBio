...
class MeshDef(object):
    ...
    def __init__(self,mfile=None,mformat='Gmsh',scale=[1.0,1.0,1.0],physicalEntities=None,print={'nodes':False,'elements':False,'elsets':False,'fsets':False, 'nsets':False},quiet=False):
        self.mfile = mfile
    ...
    def addMeshData(self,tree):
        self.meshData.append(tree)

    def getNodeTree(self):
        tree = ET.Element('Nodes', name='Object1') #Just 1 object 4 now.
        for inode,node in self.nodes.items():
            nodeElem = ET.Element('node', id=str(inode))
            nodeElem.text = str(",".join(list(map(str, node))))
            tree.append(nodeElem)
        return tree
    
    def getElementTree(self):
        elementTrees = []
        for setName,elementSet in self.elsets.items():
            elementTrees.append(ET.Element('Elements', type=elementSet['type'], name=setName))
            for ielem in elementSet['elements']:
                nodeElem = ET.Element('elem', id=str(ielem+1))
                nodeElem.text = ",".join(list(map(str, self.elements[ielem]['nodes'])))
                elementTrees[-1].append(nodeElem)

        return elementTrees
    
    
    def getSurfaceTree(self):
        surfaceTrees = []
        for setName,elementSet in self.fsets.items():
            surfaceTrees.append(ET.Element('Surface', name=setName))
            for ielem,elem in enumerate(elementSet['elements']):
                nodeElem = ET.Element(elementSet['type'], id=str(ielem+1))
                nodeElem.text = ",".join(list(map(str, elem)))
                surfaceTrees[-1].append(nodeElem)

        return surfaceTrees        

    def addSurfacePair(self, name:str = None, primary:str = None, secondary:str = None):
        self.surfacePairs[name]=[primary,secondary]

    def getNodesetTree(self):
        nodesetTrees = []
        for setName,nodeSet in self.nsets.items():
            nodes = ','.join([str(i) for i in nodeSet['nodes']])
            #nodes = ','.join(nodeSet['nodes'])
